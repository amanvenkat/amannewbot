...
const util = {
    ...
    calcMulti: async function (message) {
        ...
        if (message.channel.name.toLowerCase().includes('medu-wada')) {
            total += 0.5;
            text += 'Medu wada channel\n';
            unlocked += 1;
        }
        if (message.channel.name.toLowerCase().includes('🍩')) {
            total += 0.5;
            text += 'Medu wada Emoji\n';
            unlocked += 1
        }    
        ...
        if (donor === true) {
            total += 10;
            text += '[Donor](https://www.youtube.com/channel/UC5DqQh9__HKLd_HpDAXxsVw)\n';
            unlocked += 1;
        }
